...
function LoggedIn() {
  ...
  async function insertPassword() {
    ...
    try {
      ...
      if (editing.isEditing) {
        await axios.put(
          "/api/passwordManager/" + editing.editingPasswordId,
          passwordOptions
        );
      } else {
  ...
  for (let i = 0; i < passwordList.length; i++) {
    passwordListElement.push(
      <li>
        <span>URL: {passwordList[i].URL}</span>
        <span> || Password: {passwordList[i].Password}</span>
        <button onClick={() => deletePassword(passwordList[i]._id)}>
          Delete
        </button>
    ...
    PasswordGeneratorAssistComponent = (
      <div>
        ...
        <div>
          ...
          {lengthError && (
            <div style={{ color: "red" }}>
              Password can only be a length between 4-50 inclusive.
            </div>
          )}
        </div>
      </div>
              ...
              <div className="management-box">
                ...
                <div>
                  <div>
                    ...
                    <input
                      value={passwordURL}
                      onInput={(event) => updatePasswordURL(event)}
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      onInput={(event) => updatePasswordPassword(event)}
                    />
                  </div>
                  {PasswordGeneratorAssistComponent}
                  <div></div>
